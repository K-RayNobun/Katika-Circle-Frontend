...
const SponsorSection = () => {
    ...
    const handlePrevious = () => {
        console.log('Index switched to ', currentIndex - 1);
    ...
    const handleNext = () => {
        console.log('Attempt to switch index to ', currentIndex + 1);
        setCurrentIndex((prev) => 
            ...
            <div className='relative flex items-center justify-center h-[120px] space-x-1 lg:space-x-8 px-3'>
                ...
                <FaChevronLeft className={`text-[20px] lg:text-[26px] 
                    ${window.innerWidth >= 1024 && sponsorsImgUrlList.length <= 6 
                     || sponsorsImgUrlList.length <= 2 && currentIndex === 0 ? 'lg:hidden' : 'lg:block'}`} />
                </button>

                <div className="relative w-screen lg:min-w-[1300px] h-[240px] overflow-hidden">
                <div 
                    className={`flex items-center justify-center absolute transition-transform duration-500 ease-out h-full`}
                    style={{
                        transform: `translateX(-${window.innerWidth >= 1024 ? currentIndex * 350 : currentIndex * 340}px)`
                    }}
                >
                    {sponsorsImgUrlList.map((url, index) => (
                    <div key={index} className="h-[60px] lg:h-[75px] w-[160px] lg:w-[210px] mx-[5px] flex justify-center flex-shrink-0 bg-purple-900/60 rounded-lg">
                        <Image 
                        src={url}
                        alt={`Sponsor ${index + 1}`}
                        width={10}
                ...
                <FaChevronRight className={`text-[20px] lg:text-[26px] 
                    ...
                    || sponsorsImgUrlList.length === 2 || currentIndex === sponsorsImgUrlList.length / 2 - 1 ? 'hidden' : 'block'}`}/>
                </button>
            </div>
